...
public class TestCase extends TestExecutable {
	...
	protected void initFromIFile() {
		...
		Document doc = XMLHelper.readDocument(iFile, false); //TODO enable validation here
		Element root = doc.getRootElement();
		Namespace ns = root.getNamespace();

		// check that root element has correct name
		Assert.isTrue(root.getName().equals("TestCase"),"Root element is not TestCase");
		...
		List<Element> testStepElements =(List<Element>) root.getChildren("TestStep", ns);
